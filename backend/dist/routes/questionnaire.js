...
router.post("/", auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const response = await service.recordResponse(driver.id, // Use the actual driver.id
        safeVehicleId, status, gpsLatitude ? Number.parseFloat(gpsLatitude) : undefined, gpsLongitude ? Number.parseFloat(gpsLongitude) : undefined, timestamp ? new Date(timestamp) : undefined);
...
router.get("/driver/:driverId", auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const responses = await service.getByDriver(resolvedDriverId, limit ? Number.parseInt(limit) : 100);
