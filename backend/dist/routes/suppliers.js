...
router.get('/metrics', async (req, res) => {
    try {
        const activeRes = await database_1.default.query(`SELECT COUNT(*) FROM suppliers WHERE status = 'Ativo'`);
        const activeContractsRes = await database_1.default.query(`SELECT COUNT(*) FROM supplier_contracts WHERE status = 'Vigente'`);
        const expiringContractsRes = await database_1.default.query(`
      SELECT COUNT(*) FROM supplier_contracts
      WHERE end_date <= NOW() + INTERVAL '30 days' AND status != 'Encerrado'
    `);
        const monthPaymentsRes = await database_1.default.query(`
      SELECT COALESCE(SUM(amount), 0) AS total FROM expenses
      WHERE date >= date_trunc('month', CURRENT_DATE)
    `);
        const totalSpentYearRes = await database_1.default.query(`SELECT COALESCE(SUM(total_spent), 0) AS total FROM suppliers`);
        const avgRatingRes = await database_1.default.query(`SELECT COALESCE(AVG(rating), 5.0) AS avg FROM suppliers WHERE status = 'Ativo'`);
        res.json({
...
router.get('/charts', async (req, res) => {
    try {
        // Top 10 by spent
        const spentRes = await database_1.default.query(`
    ...
    `);
        ...
        const catRes = await database_1.default.query(`
...
router.get('/insights', async (req, res) => {
    try {
        const topBillingRes = await database_1.default.query(`
      SELECT trade_name AS name, total_spent AS amount
      FROM suppliers
      ORDER BY total_spent DESC
      LIMIT 1
    `);
        const topRatedRes = await database_1.default.query(`
      SELECT trade_name AS name, rating, services_count
      FROM suppliers
      ORDER BY rating DESC, services_count DESC
      LIMIT 1
    `);
        const criticalContractRes = await database_1.default.query(`
...
router.get('/categories', async (req, res) => {
    try {
        const result = await database_1.default.query(`SELECT * FROM supplier_categories ORDER BY name ASC`);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/categories', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO supplier_categories (name, description) VALUES ($1, $2) ON CONFLICT (name) DO UPDATE SET description = $2 RETURNING *`, [name, description || null]);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const supRes = await database_1.default.query(`SELECT * FROM suppliers WHERE id = $1`, [id]);
        ...
        const [contractsRes, contactsRes, documentsRes, historyRes, workOrdersRes, expensesRes] = await Promise.all([
            database_1.default.query(`SELECT * FROM supplier_contracts WHERE supplier_id = $1 ORDER BY end_date DESC`, [id]),
            database_1.default.query(`SELECT * FROM supplier_contacts WHERE supplier_id = $1 ORDER BY is_primary DESC, name ASC`, [id]),
            database_1.default.query(`SELECT * FROM supplier_documents WHERE supplier_id = $1 ORDER BY created_at DESC`, [id]),
            database_1.default.query(`SELECT * FROM supplier_history WHERE supplier_id = $1 ORDER BY created_at DESC`, [id]),
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const { code, trade_name, corporate_name, cnpj, state_registration, municipal_registration, primary_category, specialty, categories, status, is_homologated, rating, contact_name, phone, whatsapp, email, website, zip_code, street, number, complement, neighborhood, city, state, country, units, preferred_payment_method, payment_terms_days, bank_info, pix_key, financial_notes, notes } = req.body;
...
router.put('/:id', async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const { id } = req.params;
        const { trade_name, corporate_name, cnpj, state_registration, municipal_registration, primary_category, specialty, categories, status, is_homologated, rating, contact_name, phone, whatsapp, email, website, zip_code, street, number, complement, neighborhood, city, state, country, units, preferred_payment_method, payment_terms_days, bank_info, pix_key, financial_notes, notes } = req.body;
...
router.delete('/:id/contracts/:contractId', async (req, res) => {
    try {
        ...
        await database_1.default.query('DELETE FROM supplier_contracts WHERE id = $1', [contractId]);
        res.json({ success: true });
    }
    catch (error) {
