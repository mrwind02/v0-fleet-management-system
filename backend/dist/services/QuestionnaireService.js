...
class QuestionnaireService {
    async recordResponse(driverId, vehicleId, status, gpsLatitude, gpsLongitude, timestampResponse = new Date()) {
        const result = await (0, database_1.query)(`INSERT INTO driver_questionnaire (driver_id, vehicle_id, status, gps_latitude, gps_longitude, timestamp_response)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [driverId, vehicleId, status, gpsLatitude, gpsLongitude, timestampResponse]);
    ...
    async getLatestByDriver(driverId) {
       ...
       FROM driver_questionnaire 
       WHERE driver_id = $1 
       ORDER BY timestamp_response DESC LIMIT 1`, [driverId]);
