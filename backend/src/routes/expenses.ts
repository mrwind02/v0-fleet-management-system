...
router.get('/', async (req, res) => {
  try {
    const {
      search, category, unit, vehicle_id, driver_id, cost_center,
...
router.get('/insights', async (req, res) => {
  try {
    const [topCatRes, topVehRes, topDriverRes, totalRes] = await Promise.all([
      ...
      `),
      pool.query(`SELECT SUM(amount) as total FROM expenses WHERE status != 'Cancelada'`)
    ]);
  ...
  try {
    ...
    const [expRes, attachmentsRes, historyRes] = await Promise.all([
      pool.query(`
        ...
        WHERE e.id = $1
      `, [id]),
      pool.query(`SELECT * FROM expense_attachments WHERE expense_id = $1 ORDER BY created_at DESC`, [id]),
      pool.query(`SELECT * FROM expense_history WHERE expense_id = $1 ORDER BY created_at DESC`, [id]),
    ]);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      unit_name, vehicle_id, driver_id, cost_center, supplier, city, responsible,
      is_reimbursable, reimbursement_amount, reimbursement_payee, reimbursement_due_date,
      notes, project, has_attachment
    } = req.body;
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (driver_id !== undefined) {
      if (driver_id) {
        const dRes = await client.query('SELECT name FROM drivers WHERE id = $1', [driver_id]);
        if (dRes.rows.length > 0) driver_name = dRes.rows[0].name;
      } else {
        driver_name = null;
      }
    }
